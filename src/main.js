...
let aEventsInstances = [],
    aHamtarawEvents = require.context('./Event', true, /\.js$/).keys(),
    aProjectEvents = require.context('../../../../src/Event', true, /\.js$/).keys();

for (let i = 0; i < aHamtarawEvents.length; i++) {
    let found = aHamtarawEvents[i].match(/\.\/(.+\.js)/),
        path = found[1] || '',
